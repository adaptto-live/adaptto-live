...
describe('talks store break handling', () => {
  beforeEach(() => {
  ...
  test('generates stable break ids with per-day ordinals', async () => {
  ...
  test('break entries use the entry text as title and carry no url', async () => {
    const store = useTalksStore()
    await store.fill()

    const coffeeBreak = store.talks.find(talk => talk.id === '2023-break-day-1-1')
  ...
  test('breaks are kept in schedule order alongside talks', async () => {
    const store = useTalksStore()
    await store.fill()
